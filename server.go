...
func callWebHook(wp *WebhookPayload, url string, id string, domain string, emailDate string, secret string) error {
	...
	req.Header.Set("Mw-Date", emailDate)
...
func mailHandler(origin net.Addr, from string, to []string, in []byte) error {
	...
	if err := callWebHook(&data, endpoint, id, domain, emailDate, secret); err != nil {
